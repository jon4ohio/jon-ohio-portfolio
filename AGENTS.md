...
- **Stack/scope:** Single Next.js 16 (App Router) static portfolio site. No backend, DB, or external services — all content is static in `lib/`. `npm run dev` (webpack, port 3000) is the only long-running service; the update script already runs `npm ci` and installs the Playwright chromium browser.
